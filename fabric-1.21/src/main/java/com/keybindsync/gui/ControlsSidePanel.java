...
public class ControlsSidePanel {
    ...
    public void buildWidgets(KeyBindsScreen screen) {
        ...
        listBot = Math.min(listBot, screen.height - 20);
    ...
    private void drawProfiles(GuiGraphics g, Font font, int mx, int my) {
        ...
        if (profiles.size() > visRows) {
            ...
            String hint = scroll + "/" + max;
            g.drawString(font, hint, panelX + W - font.width(hint) - 2,
    ...
    // ── widget drawing ────────────────────────────────────────────────────────
    private static void drawTab(GuiGraphics g, Font font,
        ...
        int ty = y + (h - 8) / 2;
        g.drawString(font, label, tx, ty, active ? WHITE : (hov ? LGRAY : MGRAY));
    }

    private static void drawBtn(GuiGraphics g, Font font,
                                int x, int y, int w, int h,
        ...
        int tx = x + (w - font.width(label)) / 2;
        g.drawString(font, label, tx, y + (h - 8) / 2, hov ? WHITE : LGRAY);
    }

    private static void drawToggle(GuiGraphics g, Font font,
                                   int x, int y, int w, int h,
    ...
    private static void drawField(GuiGraphics g, Font font,
                                  int x, int y, int w, int h,
