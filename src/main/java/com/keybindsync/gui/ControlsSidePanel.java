...
/**
 * Sidebar panel drawn on the vanilla Controls screen via Fabric Screen Events.
 *
 * Porting to MC 1.21.x / older:
 *   - Replace GuiGraphicsExtractor with GuiGraphics in draw()
 *   - In KeybindSyncMod, swap afterExtract → afterRender and update event signatures
 *   - Mouse/key event objects differ; update handleClick / handleKeyPress accordingly
 */
public class ControlsSidePanel {
    ...
    private static final int HDR_H = 18;   // height of tab-button header row
    ...
    // panel background: slightly blue-tinted dark so it reads as distinct from
    // the vanilla blurred menu background
    ...
    // ── layout (set by buildWidgets from real list bounds) ────────────────────
    private int panelX    = 0;   // left edge of panel
    private int listTop   = 36;  // Y of the KeyBindsList top  → panel content top
    private int listBot   = 200; // Y of the KeyBindsList bottom
    private int visRows   = 6;   // profile rows that fit in the list height
    ...
    public void buildWidgets(KeyBindsScreen screen) {
        ...
        // Derive position from the actual KeyBindsList bounds so the panel
        // always aligns correctly regardless of screen size or GUI scale.
        int rowRight = screen.width / 2 + 85;
        int top      = 36;
        int bot      = screen.height - 36;

        for (var w : Screens.getWidgets(screen)) {
        ...
        // Centre panel in the gap to the right of the list content
        int gap  = screen.width - rowRight;
        panelX   = rowRight + Math.max(2, (gap - W) / 2);
        panelX   = Math.min(panelX, screen.width - W - 4);

        // How many profile rows fit between listTop and listBot, leaving room
        // for the name field + 2 button rows below the list.
    ...
    // ── draw (called in afterExtract — after all vanilla widgets) ─────────────
    public void draw(GuiGraphicsExtractor g, Screen screen, int mx, int my) {
        Font font = Screens.getFont(screen);

        // ── background & border ───────────────────────────────────────────────
        int hdrTop = listTop - HDR_H - 2;          // header tab row sits just above list
        int bottom = (mode == Mode.PROFILES)
                ? listBot + 4
                : settingsBottom();
        ...
        // ── header tabs ───────────────────────────────────────────────────────
        drawTab(g, font, panelX,        hdrTop, BW,     HDR_H,
                "Profiles", mode == Mode.PROFILES, mx, my);
        drawTab(g, font, panelX + BW+3, hdrTop, W-BW-3, HDR_H,
                "Settings", mode == Mode.SETTINGS, mx, my);
        ...
        // ── status text below panel ───────────────────────────────────────────
        if (!status.isEmpty())
            g.text(font, status, panelX, bottom + 3, statusColor);
    }

    // ── profiles content ──────────────────────────────────────────────────────
    private void drawProfiles(GuiGraphicsExtractor g, Font font, int mx, int my) {
        int y = listTop;
        ...
        for (int i = 0; i < visRows; i++) {
            ...
            g.text(font, name, panelX + 3, ey + (ROW_H - 8) / 2, sel ? YELLOW : LGRAY);
        }

        if (profiles.isEmpty())
            g.text(font, "No profiles yet", panelX + 3, y + 3, DGRAY);

        // scroll hint
        if (profiles.size() > visRows) {
            int max = profiles.size() - visRows;
            g.text(font, scroll + "/" + max, panelX + W - font.width(scroll + "/" + max) - 2,
        ...
        int fy = below + 4;
        // name field
        drawField(g, font, panelX, fy, W, BTN_H,
                nameBuf.toString(), "profile name…", focus == Focus.PROFILE_NAME);
    ...
    private int settingsBottom() {
        return listTop
                + (BTN_H + 2) + BTN_H + 5          // auto-save toggle + field + gap
                + (BTN_H + 2) + BTN_H + 5          // auto-load toggle + field + gap
                + BTN_H + 8                          // save settings btn + gap
                + 1 + 4 + 10                         // divider + gap + per-server label
                + BTN_H + 3 + BTN_H + 4             // srv fields row + gap + add/remove btns
                + 3 * 10 + 4;                        // rule list (3 lines) + bottom gap
    }

    private void drawSettings(GuiGraphicsExtractor g, Font font, int mx, int my) {
        ...
        y += BTN_H + 2;
        drawField(g, font, panelX, y, W, BTN_H, autoSaveBuf.toString(),
                "save-to profile…", focus == Focus.AUTO_SAVE);
        y += BTN_H + 5;
        ...
        y += BTN_H + 2;
        drawField(g, font, panelX, y, W, BTN_H, autoLoadBuf.toString(),
                "load-from profile…", focus == Focus.AUTO_LOAD);
        y += BTN_H + 5;
        ...
        y += 4;
        g.text(font, "Per-server:", panelX + 1, y, MGRAY);
        y += 10;

        drawField(g, font, panelX,        y, BW, BTN_H, srvAddrBuf.toString(),
                "server addr…", focus == Focus.SRV_ADDR);
        drawField(g, font, panelX + BW+3, y, W-BW-3, BTN_H, srvProfBuf.toString(),
                "profile…", focus == Focus.SRV_PROF);
        y += BTN_H + 3;
        ...
        for (Map.Entry<String, String> e : cfg.serverProfiles.entrySet()) {
            if (shown >= 3) { g.text(font, "…", panelX + 1, y, DGRAY); break; }
            String line = clip(e.getKey(), 9) + " → " + clip(e.getValue(), 8);
            g.text(font, line, panelX + 1, y, LGRAY);
            y += 10; shown++;
        }
        if (cfg.serverProfiles.isEmpty())
            g.text(font, "No rules yet", panelX + 1, y, DGRAY);
    }
    ...
    private void clickSettings(Screen screen, double mx, double my) {
        ...
        y += BTN_H + 8 + 1 + 4 + 10;  // save btn + divider + per-server label
    ...
    public void handleKeyPress(KeyEvent event) {
        ...
        if (key == GLFW.GLFW_KEY_BACKSPACE) {
            ...
            char c = toChar(key, (event.modifiers() & GLFW.GLFW_MOD_SHIFT) != 0);
            if (c != 0) buf.append(c);
        }
    }
    ...
    // ── widget drawing ────────────────────────────────────────────────────────
    private static void drawTab(GuiGraphicsExtractor g, Font font,
        ...
        int ty = y + (h - 8) / 2;
        g.text(font, label, tx, ty, active ? WHITE : (hov ? LGRAY : MGRAY));
    }
    ...
    private static void drawBtn(GuiGraphicsExtractor g, Font font,
        ...
        int tx = x + (w - font.width(label)) / 2;
        g.text(font, label, tx, y + (h - 8) / 2, hov ? WHITE : LGRAY);
    }

    private static void drawToggle(GuiGraphicsExtractor g, Font font,
                                   int x, int y, int w, int h,
        ...
        g.text(font, s, x + 3, y + (h - 8) / 2, WHITE);
    }

    private static void drawField(GuiGraphicsExtractor g, Font font,
                                  int x, int y, int w, int h,
        ...
        // scroll text left if too wide
        while (font.width(display) > w - 6 && display.length() > 1)
            display = display.substring(1);
        g.text(font, display, x + 3, y + (h - 8) / 2, color);
    }
